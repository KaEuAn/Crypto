...
static const byte STransform [256] = {
...
static const byte PTransform [64] = {
...
void preprocessL() {
    for(int i = 0; i < 64; ++i) {
        byte* temp = (byte*) (LTransform + i);
        for(int j = 0; j < 4; ++j) {
            swap(temp + j, temp + 7 - j);
        }
    }
}


void print(byte* x, u32 num) {
    for(int i = 0; i < num; ++i) {
        std::cout << (int) x[i] << ' ';
    }
    std::cout << '\n';
...
byte  getC(u64 i, u64 j) {
    return ConstKeySchedule[i][j];
}
...
void XorToFirst(byte* a, byte* b) {
    for(u32 i = 0; i < 64; ++i) {
        a[i] ^= b[i];
    }
}

void makeSP(byte* a, byte* result1) {
    byte result[64];
    for (u32 i = 0; i < 64; ++i)
        result[i] = STransform[a[i]];
    for (u32 i = 0; i < 64; ++i)
        result1[i] = result[PTransform[i]];
}

void makeL(byte* a, byte* result) {
    for(u32 i = 0; i < 8; ++i) {
        u64 now = 0;
        byte a_prev[8];
        for(u32 j = 0; j < 8; ++j) {
        ...
        memcpy(&now, a_prev, 8);
        u64 temp = 0;
        for(u64 j = 0, temp_byte = 1; j < 64; ++j, temp_byte <<= 1) {
            if(now & temp_byte) {
                temp ^= LTransform[63 - j];
            }
        }
        memcpy(result + i * 8, &temp, 8);
    }
}

void makeModul(byte* a, byte* b, byte* result) {
    u64 res = 0;
    for(u32 i = 63; i <= 64; --i) {
        res = a[i] + b[i] + (res >> 8);
        result[i] = static_cast<byte>(0xFF & res);
    }
}

void GenKeySchedule(byte* Key, u32 i) {
    byte result[64];
    XorToFirst(Key, ConstKeySchedule[i]);
    makeSP(Key, result); memcpy(Key, result, 64);
    makeL(Key, result); memcpy(Key, result, 64);
}

void EKey(byte* Key, byte* message, byte* result) {
    byte current[64]; //temp for answers
    memcpy(result, Key, 64);
    XorToFirst(result, message);
    for(u32 i = 0; i < 12; ++i) {
        makeSP(result, current); memcpy(result, current, 64);
        
        makeL(result, current); memcpy(result, current, 64);
        
        GenKeySchedule(Key, i);
        
            print(Key, 64);
        XorToFirst(result, Key);
    }
}

void Compression(byte* N, byte* h, byte* m, byte* result) {
    byte temp[64]; memcpy(temp, h, 64);
    XorToFirst(temp, N);
    makeSP(temp, result); memcpy(temp, result, 64);
    makeL(temp, result); memcpy(temp, result, 64);
    EKey(temp, m, result);
    XorToFirst(result, h);
    XorToFirst(result, m);
}

void Hash(byte* message, u32 len, byte* result) {
    byte h[64];
    memcpy(h, IV, 64);

    u32 cou = 0; //how many to delete
    while(len > 512) {
        byte temp[64];
        memcpy(temp, message + (len - 64), 64);
        byte answer[64];
        Compression(N, h, temp, answer); memcpy(h, answer, 64);
        makeModul(N, N512, answer); memcpy(N, answer, 64);
        makeModul(Sigma, temp, answer); memcpy(Sigma, answer, 64);
        len -= 512;
        ++cou;
    }
    u32 new_len = len / 8 - cou;
    byte new_message[64];
    memcpy(new_message + (64 - new_len), message, new_len);
    if (len/8 - cou < 64) {
        for(u32 i = 0; i < 64 - new_len - 1; ++i) {
            new_message[i] = 0;
        }
        new_message[64 - new_len - 1] = 1;
    }
    byte temp[64];
    Compression(N, h, new_message, temp); memcpy(h, temp, 64);
    makeModul(N, new_message, temp); memcpy(N, temp, 64);
    makeModul(Sigma, new_message, temp); memcpy(Sigma, temp, 64);
    Compression(N0, h, N, temp); memcpy(h, temp, 64);
    Compression(N0, h, Sigma, temp); memcpy(h, temp, 64);
    if (len == 512) {
        memcpy(result, h, 64);
    } else {
        memcpy(result, h, 32);
...
void test_right() {
    byte message[] = {0x32,0x31,0x30,0x39,0x38,0x37,0x36,0x35,0x34,0x33,0x32,0x31,0x30,0x39,0x38,0x37,
...
void test_speed() {
    byte message[] = {0x32,0x31,0x30,0x39,0x38,0x37,0x36,0x35,0x34,0x33,0x32,0x31,0x30,0x39,0x38,0x37,
                      0x36,0x35,0x34,0x33,0x32,0x31,0x30,0x39,0x38,0x37,0x36,0x35,0x34,0x33,0x32,0x31,
                      0x30,0x39,0x38,0x37,0x36,0x35,0x34,0x33,0x32,0x31,0x30,0x39,0x38,0x37,0x36,0x35,
                      0x34,0x33,0x32,0x31,0x30,0x39,0x38,0x37,0x36,0x35,0x34,0x33,0x32,0x31,0x30,0x35};
    byte result[64];
    init(512);
    
    clock_t start = clock();
    // 60 MB = 60 * 1024 B, we have message with size 64 B, so expect not more than 1 second
